...
- **A turn that fails terminally no longer ends in a bare error.** Instead of
  ...
  report. See [Async Retry Escalation](runtime/async-retry-escalation.md).
...
- **Giving up produces a report, not a shrug.** Every non-success terminal carries the
  ...
  Captain's Log. See [Request Status](runtime/request-status.md).
...
Observability events are now redacted far more thoroughly, and entity-based PII
redaction (names, addresses, organizations, dates of birth, financial
identifiers) ships as a built-in capability that is on by default.
