...
| `embedding` | Vector embeddings | `openai/text-embedding-3-large` or `local/nomic-ai/nomic-embed-text-v1.5` |
...
| Local (embeddings) | `local/{hf-repo-id}` or `local/{hf-repo-id}:{revision}` | `local/nomic-ai/nomic-embed-text-v1.5` |
